...
def OnBeginSpellCast( spell ):
	print "Charm Monster OnBeginSpellCast"
	print "spell.target_list=", spell.target_list
	print "spell.caster=", spell.caster, " caster.level= ", spell.caster_level
	game.particles( "sp-enchantment-conjure", spell.caster )

def	OnSpellEffect( spell ):
	print "Charm Monster OnSpellEffect"

	if (not spell.target_list):
		print "No targets for Charm Monster!"
	...
	if not target_item_obj.is_friendly( spell.caster ):
		...
		if not target_item_obj.saving_throw_spell( spell.dc, D20_Save_Will, D20STD_F_CHARM, spell.caster, spell.id ):
			...
			target_item_obj.condition_add_with_args( 'sp-Charm Monster', spell.id, spell.duration, target_item_obj.hit_dice_num )
			target_item.partsys_id = game.particles( 'sp-Charm Monster', target_item_obj )
...
def OnBeginRound( spell ):
	print "Charm Monster OnBeginRound"

def OnEndSpellCast( spell ):
	print "Charm Monster OnEndSpellCast"