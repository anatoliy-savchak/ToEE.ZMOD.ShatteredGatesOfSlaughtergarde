...
def	OnSpellEffect( spell ):
	...
	try:
		OnSpellEffectImpl(spell)
	except Exception, e:
		print "Shout OnSpellEffect error:", sys.exc_info()[0]
		print(str(e))
	spell.spell_end(spell.id)
	return

def	OnSpellEffectImpl( spell ):
	remove_list = []
...
#		range = 25 + 2 * int(spell.caster_level/2)
		...
		for obj in target_list:
			if not obj.saving_throw_spell( spell.dc, D20_Save_Fortitude, D20STD_F_NONE, spell.caster, spell.id ):
				...
				obj.spell_damage( spell.caster, D20DT_MAGIC, damage_dice, D20DAP_UNSPECIFIED, D20A_CAST_SPELL, spell.id )
...
#				obj.partsys_id = game.particles( 'sp-Shout-Hit', obj )
				...
				obj.spell_damage_with_reduction( spell.caster, D20DT_MAGIC, damage_dice, D20DAP_UNSPECIFIED, DAMAGE_REDUCTION_HALF, D20A_CAST_SPELL, spell.id )
#				obj.partsys_id = game.particles( 'sp-Shout-Hit', obj )
				game.particles( 'sp-Shout-Hit', obj )
